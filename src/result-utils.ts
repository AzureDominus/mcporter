...
function extractStructuredContent(raw: unknown): unknown {
	if (
		raw &&
		typeof raw === "object" &&
		"structuredContent" in (raw as Record<string, unknown>)
	) {
...
export function createCallResult<T = unknown>(raw: T): CallResult<T> {
	return {
		...
		markdown(joiner = "\n") {
			...
			const markdownEntries = content.filter(
				(entry) =>
					entry &&
					typeof entry === "object" &&
					(entry as Record<string, unknown>).type === "markdown",
			);
		...
		json<J = unknown>() {
			...
			if (parsedStructured) return parsedStructured as J;
			...
			if (content) {
				for (const entry of content) {
					if (
						entry &&
						typeof entry === "object" &&
						(entry as Record<string, unknown>).type === "json"
					) {
						const parsed = tryParseJson(entry);
						if (parsed) return parsed as J;
					}
				}
			}
			return null;
